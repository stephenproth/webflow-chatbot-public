function saveConversationHistory() {
  ...
  let lastUserMessageProcessed = false;
  conversationHistory
    .filter((message) => message.role !== "system")
    .forEach((message) => {
      if (message.role === "user" && !lastUserMessageProcessed) {
        lastUserMessageProcessed = true;
        addMessageToWidget(message.content, message.role, null, false);
        handleUserOption(message.content);
      } else if (message.role === "assistant") {
        lastUserMessageProcessed = false;
        addMessageToWidget(message.content, message.role, null, false);
      }
    });
