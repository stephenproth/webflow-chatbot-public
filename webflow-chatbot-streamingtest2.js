function saveConversationHistory() {
  ...
  function sendToPipedream(message, callback) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        addMessageToWidget(chatbotResponse, "assistant", null, function () {
  ...
  function streamText(content, index, callback) {
    if (index < content.length) {
      ...
      setTimeout(() => {
        ...
        streamText(content, index + 1, callback);
      }, delay);
    } else {
  ...
  function addMessageToWidget(content, role, hyperlink = null, callback = null) {
    if (role === "bot-options") {
    ...
    if (hyperlink) {
      ...
      messageElement.innerHTML = ""; // Initialize the message as empty
    }
    ...
    if (role === "assistant") {
      ...
      streamText(content, 0, callback);
    } else {
      // For user messages, proceed as normal
      if (callback) {
        callback();
      }
    }
