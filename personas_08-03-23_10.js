...
function generatePDF(suffix) {
    // Get the response container and clone it
    const responseContainer = document.getElementById("response-container" + suffix);
    const clone = responseContainer.cloneNode(true);
    
    // Remove CSS styles from the clone
    clone.style = "";
    Array.from(clone.getElementsByTagName("*")).forEach((el) => el.style = "");
    
    // Append the clone to the body (it will not be visible because of 'display: none')
    ...
    html2pdf().from(clone).set(opt)
      .save()
      .then(() => document.body.removeChild(clone));  // Remove the clone from the body after the PDF is saved
}
